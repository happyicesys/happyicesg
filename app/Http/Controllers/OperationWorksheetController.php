...
/*
        ...
        foreach($people as $index1 => $person) {
            foreach($dates as $index2 => $date) {
                ...
                $total = $total->select(DB::raw('(CASE WHEN transactions.gst=1 THEN(CASE WHEN transactions.is_gst_inclusive=0 THEN deals.amount*((100 + transactions.gst_rate)/100) ELSE deals.amount END) ELSE deals.amount END) AS total'))->get();
