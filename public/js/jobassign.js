...
function transController($scope, $http, $window) {
    ...
    $scope.onIsServiceClicked = function (transaction_id, driverkey, transactionkey) {
        $http.post('/api/transaction/is_service/' + transaction_id).success(function (data) {
            $scope.drivers[driverkey].transactions[transactionkey].is_service = data.is_service;
        });
    }
