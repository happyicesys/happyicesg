...
function transController($scope, $http) {
    ...
    function getPage(pageNumber) {
        ...
        $http.get('/api/transaction/excel/histories/unit-price').success(function (data) {
            $scope.excelHistoriesUnitPrice = data;
        });
    }
    ...
    $scope.uploadExcelUnitPrice = function (event) {
        event.preventDefault();
        var request = {
            method: 'POST',
            url: '/api/transaction/excel/import/unit-price',
            data: formData,
            headers: {
                'Content-Type': undefined
            }
        ...
        $http(request)
            .then(function success(e) {
                ...
                $scope.errors = [];
                // clear uploaded file
                var fileElement = angular.element('#excel_file_unit_price');
                fileElement.value = '';
                if (e.data === 'true') {
                    alert("Excel file uploaded and transactions loaded");
                } else {
